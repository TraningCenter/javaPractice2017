package avilova.lift.app.interfaces.impl;


import avilova.lift.app.Lift;
import avilova.lift.app.interfaces.IAction;
...
public class ActionDown implements IAction {
     ...
     * @param lift-лифт
     */
    public ActionDown(Lift lift) {
        this.lift = lift;
    }

    /**
     * движение лифта, подходит для тех, кто движется вниз, сначала пассажиры
     * выходят, потом заходят, если паасажир отказался, т.е. isSuit = false или
     * грузоподъемность не позвоняет ему зайти, то поле isLater помечается true
     * и лифт приедет за ним, но уже в следующий раз. Лифт поднимается, если
     * пассажир находится выше, чем лифт, иначе опускается, если пассажир
     * находится ниже лифта
     *
    ...
    /**
     * найти этаж назначения, на котором кто-либо выходит или заходит и
     * расстояние до него самое наименьшее
     *
